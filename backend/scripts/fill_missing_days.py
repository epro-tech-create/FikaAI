...
async def ensure_session(
    db, d: date, instructor: Instructor, location: PracticalLocation
):
    sess = (
        await db.execute(
            select(AttendanceSession).where(AttendanceSession.session_date == d)
        )
    ).scalar_one_or_none()
    if sess:
    ...
    sess = AttendanceSession(
        ...
        instructions=(
            "Auto-filled missing day (no deletion). Summit day: some attended Vodacom summit."
            if d == SUMMIT_DATE
            else "Auto-filled missing day (no deletion)."
        ),
...
async def main():
    ...
    async with session_factory() as db:
        ...
        for d in TARGET_DATES:
            ...
            print(
                f" {d}: session {sess.id} existing records {len(existing_ids)} / {len(students)} students"
            )
            ...
            if d == ALL_PRESENT_DATE:
                ...
                for r in rows:
                    if r.status != AttendanceStatus.PRESENT:
                        ...
                        r.check_in_at = datetime.combine(
                            d, t, tzinfo=settings.campus_tz
                        )
                        r.minutes_late = 0
            ...
            for student in to_create:
                ...
                if d == ALL_PRESENT_DATE:
                    ...
                    if r < 0.15:
                        continue  # absent
                    status = (
                        AttendanceStatus.LATE if r >= 0.85 else AttendanceStatus.PRESENT
                    )
                ...
                if random.random() < 0.88:
                    ...
                    if earliest <= latest:
                        ...
                        out_local = datetime.combine(
                            d, out_t, tzinfo=settings.campus_tz
                        )
                        check_out_at = out_local
                        time_spent = int(
                            (out_local - check_in_local).total_seconds() // 60
                        )
            ...
            await db.flush()
            print(
                f"  -> inserted {created} new records for {d} (skipped {len(students)-len(existing_ids)-created} absent)"
            )
        await db.commit()
        # summary
