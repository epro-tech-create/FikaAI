...
def upgrade() -> None:
    op.add_column(
        "face_verifications",
        sa.Column("face_enrollment_id", pg.UUID(as_uuid=True), nullable=True),
    )
    ...
    op.create_index(
        "ix_face_verifications_enrollment", "face_verifications", ["face_enrollment_id"]
    )
    ...
    op.create_index(
        "ix_attendance_records_enrollment", "attendance_records", ["face_enrollment_id"]
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "fk_attendance_records_enrollment", "attendance_records", type_="foreignkey"
    )
    ...
    op.drop_constraint(
        "fk_face_verifications_enrollment", "face_verifications", type_="foreignkey"
    )
