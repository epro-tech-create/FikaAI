...
SESSION_STATUS = sa.Enum(
    "SCHEDULED", "ACTIVE", "CLOSED", "CANCELLED", name="session_status"
)
...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_students_regno", "students", ["registration_number"], unique=True
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "location_id",
            pg.UUID(as_uuid=True),
            sa.ForeignKey("practical_locations.id", ondelete="RESTRICT"),
            nullable=False,
        ),
        ...
        sa.Column(
            "late_threshold_minutes", sa.Integer(), nullable=False, server_default="15"
        ),
    ...
    op.create_index(
        "ix_sessions_status_date", "attendance_sessions", ["status", "session_date"]
    )
