...
def upgrade() -> None:
    ...
    op.add_column(
        "attendance_records", sa.Column("excuse_reason", sa.String(500), nullable=True)
    )
    op.add_column(
        "attendance_records", sa.Column("excused_by", sa.UUID(), nullable=True)
    )
    op.add_column(
        "attendance_records",
    ...
    op.create_foreign_key(
        "fk_attendance_records_excused_by",
        "attendance_records",
        "users",
        ["excused_by"],
        ["id"],
...
def downgrade() -> None:
    op.drop_constraint(
        "fk_attendance_records_excused_by", "attendance_records", type_="foreignkey"
    )
