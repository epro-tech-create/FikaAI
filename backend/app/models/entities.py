...
from sqlalchemy import (Float, ForeignKey, Index, Integer, LargeBinary,
                        Numeric, String, Time, UniqueConstraint, func, text)
...
def _enum(e: type[enum.Enum], name: str) -> SAEnum:
    return SAEnum(
        e, name=name, values_callable=lambda x: [m.value for m in x], native_enum=True
    )
...
class Student(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    status: Mapped[StudentStatus] = mapped_column(
        _enum(StudentStatus, "student_status"),
        nullable=False,
...
class PracticalLocation(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    location_type: Mapped[LocationType] = mapped_column(
        _enum(LocationType, "location_type"), nullable=False
    )
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)

    __table_args__ = (
        CheckConstraint("radius_meters > 0", name="ck_location_radius_positive"),
    )
...
class AttendanceSession(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    location_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("practical_locations.id", ondelete="RESTRICT"),
        nullable=False,
    )
    ...
    status: Mapped[SessionStatus] = mapped_column(
        _enum(SessionStatus, "session_status"), nullable=False
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "permitted_radius_meters > 0", name="ck_session_radius_positive"
        ),
    )
...
class FaceVerification(UUIDPrimaryKeyMixin, Base):
    ...
    __table_args__ = (
        Index("ix_faceverif_student_session", "student_id", "session_id"),
    )
...
class VenueVerification(UUIDPrimaryKeyMixin, Base):
    ...
    __table_args__ = (
        Index("ix_venueverif_student_session", "student_id", "session_id"),
    )
...
class AttendanceRecord(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    status: Mapped[AttendanceStatus] = mapped_column(
        _enum(AttendanceStatus, "attendance_status"), nullable=False
    )
    verification_method: Mapped[VerificationMethod] = mapped_column(
        _enum(VerificationMethod, "verification_method"),
        nullable=False,
        default=VerificationMethod.FACE_GPS,
    )
    source: Mapped[RecordSource] = mapped_column(
        _enum(RecordSource, "record_source"),
        nullable=False,
        default=RecordSource.ONLINE,
    )
    ...
    excused_by: Mapped[uuid.UUID | None] = mapped_column(
...
class Conversation(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    type: Mapped[ConversationType] = mapped_column(
        _enum(ConversationType, "conversation_type"),
        nullable=False,
    ...
    messages: Mapped[list["ChatMessage"]] = relationship(
        cascade="all, delete-orphan", lazy="selectin", order_by="ChatMessage.created_at"
    )
