...
from app.models.entities import (ChatMessage, Conversation,
                                 ConversationParticipant, ConversationType,
                                 User)
...
async def list_conversations(
    ...
    for conv in convs:
        ...
        # title / peer
        peer = (
            await _other_user_display(db, conv, user.id)
            if conv.type == ConversationType.DIRECT
            else None
        )
        title = (
            conv.title
            if conv.type == ConversationType.GROUP and conv.title
            else (peer["fullName"] if peer else "Chat")
        )
        result.append(
            {
                "id": str(conv.id),
                ...
                "participants": [
                    {
                        ...
                        "lastReadAt": (
                            p.last_read_at.isoformat() if p.last_read_at else None
                        ),
                    }
                    for p in conv.participants
                ],
                "lastMessage": (
                    {
                        "id": str(last_msg.id),
                        "body": last_msg.body,
                        "senderId": str(last_msg.sender_id),
...
async def create_conversation(
    ...
    participant_ids_raw = (
        payload.get("participantUserIds")
    ...
    for uid in all_ids:
        db.add(
            ConversationParticipant(
                ...
                user_id=uid,
                last_read_at=_now() if uid == user.id else None,
            )
        )
    await db.flush()
    await db.commit()
...
async def list_messages(
    ...
    q = (
        select(ChatMessage)
        .where(ChatMessage.conversation_id == cid, ChatMessage.deleted_at == None)
        .order_by(ChatMessage.created_at.desc())
    )
    if before:
        try:
            ...
            return [
                {
                    ...
                    "senderName": m.sender.full_name if m.sender else "",
                    "replyToId": str(m.reply_to_id) if m.reply_to_id else None,
...
async def send_message(
    ...
    if idem:
        try:
            ...
            existing = (
                await db.execute(
                    select(ChatMessage).where(ChatMessage.idempotency_key == idem_uuid)
                )
            ).scalar_one_or_none()
            if existing:
                return {
                    "id": str(existing.id),
                    "body": existing.body,
                    "senderId": str(existing.sender_id),
