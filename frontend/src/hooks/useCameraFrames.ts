...
export function useCameraFrames() {
  ...
  async function start() {
    ...
    try {
      stream.current = await withTimeout(
        mediaPromise,
        "Camera startup timed out. Close other camera apps and try again.",
      );
    } catch (error) {
      mediaPromise
        .then((lateStream) =>
    ...
    if (!track) {
      stop();
      throw new Error("No camera video track was provided.");
    }
    settings.current = track.getSettings();
    track.addEventListener("ended", () => {
      if (!stopping.current)
        problem.current = new Error(
          "Camera disconnected. Reopen the scanner and try again.",
        );
    });
    track.addEventListener("mute", () => {
      if (mutedTimer.current) window.clearTimeout(mutedTimer.current);
      mutedTimer.current = window.setTimeout(() => {
        if (!stopping.current && track.muted)
          problem.current = new Error(
            "Camera video paused. Check camera access and try again.",
          );
      }, 1500);
    });
    ...
    try {
      await withTimeout(
        (async () => {
          for (let i = 0; i < 60 && !video.current; i++)
  ...
  function grabFrame(afterVideoTime = -1) {
    ...
    if (element.currentTime <= afterVideoTime)
      throw new Error("A fresh camera frame is not available yet.");
