...
export type FaceReading = ReadingMetadata &
  LightingAssessment & {
    faceCount: number;
    centered: boolean;
    sizeOk: boolean;
...
function loadLandmarker() {
  if (!landmarkerPromise) {
    landmarkerPromise = FilesetResolver.forVisionTasks("/mediapipe")
      .then((fileset) =>
...
function measureLighting(
  ...
  const sourceWidth = Math.min(
    ...
    (bounds.width + paddingX * 2) * video.videoWidth,
  );
  const sourceHeight = Math.min(
    video.videoHeight - sourceY,
    (bounds.height + paddingY * 2) * video.videoHeight,
  );
  canvas.width = 40;
  canvas.height = 48;
  const context = canvas.getContext("2d", { willReadFrequently: true });
  if (!context || sourceWidth <= 0 || sourceHeight <= 0)
    return assessFaceLighting(new Uint8ClampedArray());
  context.drawImage(
    video,
    sourceX,
    sourceY,
...
function toReading(
  ...
  if (faceCount !== 1) {
    return {
      ...
      hint:
        faceCount > 1
          ? "Only one person may be in the scanner"
          : EMPTY_READING.hint,
    };
  }
  ...
  return {
    ...
    pitch,
    blink: Math.max(
      categories.get("eyeBlinkLeft") || 0,
      categories.get("eyeBlinkRight") || 0,
    ),
    smile: Math.max(
      categories.get("mouthSmileLeft") || 0,
      categories.get("mouthSmileRight") || 0,
    ),
    hint,
  };
}
  ...
  async function start() {
    ...
    const analyze = () => {
      ...
      if (
        ...
        const landmarks = result.faceLandmarks[0];
        if (!lightingCanvas.current)
          lightingCanvas.current = document.createElement("canvas");
        const lighting = landmarks
          ? measureLighting(element, landmarks, lightingCanvas.current)
          : assessFaceLighting(new Uint8ClampedArray());
