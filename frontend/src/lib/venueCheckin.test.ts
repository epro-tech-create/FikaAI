...
describe("QR check-in routing", () => {
  ...
  it("sends a QR login to GPS check-in, not Face ID", () => {
    expect(
      studentPostAuthPath("/student/checkin?code=A7K9P2X4", "A7K9P2X4"),
    ).toBe("/student/checkin?code=A7K9P2X4");
    expect(studentPostAuthPath(null, "A7K9P2X4")).toBe(
      "/student/checkin?code=A7K9P2X4",
    );
  });

  it("still uses a stored venue code when login drops the next query", () => {
    storeVenueCode("A7K9P2X4");
    expect(studentPostAuthPath(null, "")).toBe(
      "/student/checkin?code=A7K9P2X4",
    );
  ...
  it("sends a normal student login to Face ID attendance", () => {
    ...
    expect(studentPostAuthPath("/admin/dashboard", "")).toBe(
      "/student/attendance",
    );
  });
  ...
  it("builds the student check-in path from a code", () => {
    expect(studentCheckinPath("a7k9p2x4")).toBe(
      "/student/checkin?code=A7K9P2X4",
    );
  });

  it("sends QR signup to check-in and normal signup to Face ID enrolment", () => {
    expect(studentPostSignupPath("A7K9P2X4")).toBe(
      "/student/checkin?code=A7K9P2X4",
    );
  ...
  it("does not treat /student/attendance as a venue code", () => {
    expect(studentPostAuthPath("/student/attendance", "")).toBe(
      "/student/attendance",
    );
