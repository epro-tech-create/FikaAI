...
describe("application configuration", () => {
  ...
  it("maps every application to its exclusive role, home route and title", () => {
    expect(applicationConfig("student")).toMatchObject({
      role: "student",
      home: "/student/attendance",
      title: "Student App",
    });
    expect(applicationConfig("admin")).toMatchObject({
      role: "admin",
      home: "/admin/dashboard",
      title: "Administrator Console",
    });
    expect(applicationConfig("instructor")).toMatchObject({
      role: "instructor",
  ...
  it("normalizes cross-application login URLs", () => {
    expect(
      externalLoginUrl(
        "https://instructor.example.com/base/?from=home",
        "https://fallback.example.com",
      ),
    ).toBe("https://instructor.example.com/login");
    expect(externalLoginUrl("", "http://localhost:5175")).toBe(
      "http://localhost:5175/login",
    );
    expect(() =>
      externalLoginUrl("javascript:alert(1)", "https://fallback.example.com"),
