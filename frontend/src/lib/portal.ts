...
const adminLabels = ['Dashboard', 'Students', 'Instructors', 'Face Enrolments', 'Reports', 'Users and Roles', 'Audit Logs', 'System Settings', 'Profile'] as const
const instructorLabels = ['Dashboard', 'Live Attendance', 'Student Attendance', 'Reports', 'Notifications', 'Profile'] as const
...
export function portalNavigation(role: Extract<Role, 'admin' | 'instructor'>): NavItem[] {
  ...
  const marks = role === 'admin' ? ['DB', 'ST', 'IN', 'FE', 'RE', 'UR', 'AL', 'SS', 'PR'] : ['DB', 'LA', 'SA', 'RE', 'NO', 'PR']
