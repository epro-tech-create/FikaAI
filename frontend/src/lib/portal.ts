...
const adminLabels = ['Dashboard', 'Students', 'Instructors', 'Courses', 'Face Enrolments', 'Reports', 'Users and Roles', 'Audit Logs', 'System Settings', 'Profile'] as const
const instructorLabels = ['Dashboard', 'My Courses', 'Live Attendance', 'Student Attendance', 'Reports', 'Notifications', 'Profile'] as const
...
export function portalNavigation(role: Extract<Role, 'admin' | 'instructor'>): NavItem[] {
  ...
  const marks = role === 'admin' ? ['DB', 'ST', 'IN', 'CO', 'FE', 'RE', 'UR', 'AL', 'SS', 'PR'] : ['DB', 'MC', 'LA', 'SA', 'RE', 'NO', 'PR']
