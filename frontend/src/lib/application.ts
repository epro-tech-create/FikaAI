...
const APPLICATIONS: Record<Application, ApplicationConfig> = {
  student: {
    ...
    title: "Student App",
    eyebrow: "FIELD PRACTICAL ATTENDANCE",
  },
  admin: {
    role: "admin",
...
export function resolveApplication(value: unknown): Application {
  ...
  if (!application)
    throw new Error(
      `Unknown application "${String(value)}". Expected one of: student, admin, instructor.`,
    );
  return application;
}
...
export function externalLoginUrl(
  value: unknown,
  fallbackOrigin: string,
): string {
  const raw =
    typeof value === "string" && value.trim() ? value.trim() : fallbackOrigin;
...
export function instructorLoginUrl(): string {
  const fallback = import.meta.env.DEV
    ? "http://localhost:5175"
    : "https://attendance.cyberclubdit.org";
