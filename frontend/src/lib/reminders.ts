...
export function checkRemindersTick() {
  ...
  if (h === "08:00")
    pushReminder({
      id: `open-${today}`,
      title: "Check-in open",
      body: "Check-in is now open until 15:00. Arrive by 09:30 to be early.",
      time: new Date().toISOString(),
    });
  if (h === "09:15")
    pushReminder({
      id: `cutoff-${today}`,
      title: "Late cutoff in 15 min",
      body: "Arrive before 09:30 to be PRESENT. After 09:30 you are LATE.",
      time: new Date().toISOString(),
    });
  if (h === "14:55")
    pushReminder({
      id: `checkout-soon-${today}`,
      title: "Checkout in 5 min",
      body: "Checkout opens at 15:00. Stay until 17:00.",
      time: new Date().toISOString(),
    });
  if (h === "15:00")
    pushReminder({
      id: `checkout-${today}`,
      title: "Checkout open",
      body: "You can now check out until 17:00.",
      time: new Date().toISOString(),
    });
}
