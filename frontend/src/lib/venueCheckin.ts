...
export function readStoredVenueCode(): string {
  return normalizeVenueCode(
    typeof sessionStorage === "undefined"
      ? ""
      : sessionStorage.getItem(VENUE_CODE_KEY),
  );
}
...
export function studentCheckinPath(code?: string): string {
  ...
  return venue
    ? `/student/checkin?code=${encodeURIComponent(venue)}`
    : "/student/checkin";
}
...
export function studentLoginPath(nextPath?: string, code?: string): string {
  const venue =
    normalizeVenueCode(code) ||
    extractVenueCode(nextPath) ||
    readStoredVenueCode();
...
export function loginPathPreservingVenue(currentPath?: string): string {
  ...
  if (venue || /\/checkin(?:\?|$)/.test(here)) {
    const next = here.includes("/checkin")
      ? safeNextPath(here) || studentCheckinPath(venue)
      : studentCheckinPath(venue);
...
export function studentPostAuthPath(
  ...
  const venue =
    normalizeVenueCode(codeFromQuery) ||
    extractVenueCode(next) ||
    readStoredVenueCode();
  if (venue) {
