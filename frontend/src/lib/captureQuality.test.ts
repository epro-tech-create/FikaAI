...
describe("parseChallengeType", () => {
  ...
  it.each([undefined, null, "", "blink_twice", "UNKNOWN", 42])(
    "rejects missing or unknown value %s",
    (value) => {
      expect(() => parseChallengeType(value)).toThrow(
        /unsupported face challenge/i,
      );
    },
  );
});
...
describe("face-region lighting", () => {
  ...
  it("accepts a moderately bright crop with useful contrast", () => {
    const assessment = assessFaceLighting(
      grayscalePixels([75, 100, 130, 165, 190]),
    );
