...
export default function SessionCountdown({ session }: { session:any }) {
  ...
  const openLabel = session.checkInOpen || '08:00'
  const cutLabel = session.officialStart || '09:30'
  const closeLabel = session.checkInClose || '15:00'
  const outOpenLabel = session.expectedEnd || session.checkoutOpensAt?.slice(11, 16) || '15:00'
  const outCloseLabel = session.checkOutClose || '17:00'
  if (now < open) { state=`Opens at ${openLabel}`; color='#6b8590' }
  else if (now < cut) { state=`Early until ${cutLabel} · ${cut-now} min left`; color='#0ea5e9' }
  else if (now < close) { state=`Late from ${cutLabel} · Closes ${closeLabel}`; color='#f59e0b' }
  else if (now < outOpen) { state=`Check-in closed · Checkout ${outOpenLabel}`; color='#6b8590' }
  else if (now < outClose) { state=`Checkout open until ${outCloseLabel}`; color='#10b981' }
