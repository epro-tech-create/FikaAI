...
export function CardToolbar({
  ...
  return (
    <div className="card-toolbar">
      <div className="card-toolbar-title">
        <b>{title}</b>
        <span>{meta}</span>
      </div>
...
export function StatePanel({
  ...
  return (
    <div className={`state-panel ${kind}`}>
      {kind === "loading" && <i />}
      <b>
        {kind === "loading"
      ...
      <span>
        {children ||
          (kind === "loading"
            ? "Connecting to the attendance service..."
...
export function DataTable({
  ...
  renderActions?: ReactNode | ((item: Record<string, unknown>) => ReactNode);
}) {
  return (
    <div
      ...
      <table className="data-table">
        ...
        <tbody>
          {items.map((item, index) => (
            <tr key={String(item.id ?? item.sessionId ?? index)}>
                ...
                return (
                  <td key={column.key} data-label={column.label}>
                    {columnIndex === 0 ? (
                      ...
                      <span
                        className={`status-pill ${value === false ? "inactive" : ""}`}
                      >
                        {displayValue(value)}
                      </span>
                    ) : (
...
export function StatCard({
  ...
  return (
    <article className={`stat-card ${tone}`}>
      <span>{label}</span>
      <b>{value === undefined || value === null ? "0" : String(value)}</b>
      <small>{note}</small>
    </article>
  );
}
