...
api.interceptors.request.use((config) => {
  const token = localStorage.getItem('ccd.access') ?? localStorage.getItem('fikaai.access')
  if (token) config.headers.Authorization = `Bearer ${token}`
  // MAC/device filtering: bind every attendance request to the registered device
  try {
    const deviceId = localStorage.getItem('ccd.registration-device') ?? localStorage.getItem('fikaai.registration-device')
    if (deviceId) {
