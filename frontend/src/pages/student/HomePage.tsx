...
function getMonToFri(): { iso: string; label: string; weekday: string }[] {
  ...
  return [0, 1, 2, 3, 4].map((i) => {
    ...
    const campusIso = new Date(d).toLocaleDateString("en-CA", {
      timeZone: "Africa/Dar_es_Salaam",
    });
...
function WeeklyGraph({ calendar, streak }: { calendar: any[]; streak?: any }) {
  ...
  const visiblePoints = useMemo(
    () => points.filter((p) => p.iso <= todayIso),
    [points, todayIso],
  );
  ...
  function buildAreaPath(getVal: (p: any) => number | null): string {
    ...
    const lastIdx = (() => {
      for (let i = displayPoints.length - 1; i >= 0; i--)
        if (getVal(displayPoints[i]) != null) return i;
      return -1;
    })();
    ...
    <div className="home-graph-card">
      ...
      <div className="home-graph-top">
        ...
        <div className="home-graph-legend">
          <span className="legend-item checked-in">
            <i /> Check-in
          </span>
          <span className="legend-item checked-out">
            <i /> Check-out
          </span>
          <span className="legend-avg">
            {avgIn != null ? `Avg ${shortMinutes(avgIn)}` : "No data yet"}
          </span>
        </div>
      </div>
      ...
      <div className="home-graph-bars">
        <div className="mini-bar">
          <span className="dot green" /> <b>Arrivals</b>{" "}
          <small>
            {displayPoints.filter((p) => p.inMin != null).length}/
            {displayPoints.length} this week
          </small>
          <div className="bar-track">
            <i
              style={{
                width: `${displayPoints.length ? (displayPoints.filter((p) => p.inMin != null).length / displayPoints.length) * 100 : 0}%`,
                background: "#22c55e",
              }}
            />
          </div>
        ...
        <div className="mini-bar amber">
          <span className="dot amber" /> <b>Departures</b>{" "}
          <small>
            {displayPoints.filter((p) => p.outMin != null).length}/
            {displayPoints.length}
          </small>
          <div className="bar-track">
            <i
              style={{
                width: `${displayPoints.length ? (displayPoints.filter((p) => p.outMin != null).length / displayPoints.length) * 100 : 0}%`,
                background: "#f59e0b",
              }}
            />
          </div>
      ...
      <div className="home-graph-wrap">
        {hasAny ? (
          <svg
            ...
            role="img"
            aria-label="Weekly arrival time graph"
          >
            {/* grid */}
            {yLabels.map((yVal) => {
              const y = yFor(yVal)!;
            ...
            {displayPoints.map((p, i) => {
              ...
              return (
                <g key={`in-${p.iso}`}>
                  <circle cx={x} cy={y} r={isToday ? 6 : 4} className="pt in" />
            ...
            {displayPoints.map((p, i) => {
              ...
              return (
                <g key={`out-${p.iso}`}>
                  <circle cx={x} cy={y} r={4} className="pt out" />
                </g>
              );
            })}
            ...
            {yLabels.map((yVal) => {
              const y = yFor(yVal)!;
              return (
                <text key={yVal} x={padL - 8} y={y + 3} className="y-label">
                  {yLabelText(yVal)}
                </text>
              );
            })}
            ...
            {displayPoints.map((p, i) => {
              ...
              return (
                <text
                  ...
                  x={x}
                  y={H - 8}
                  className={`x-label ${isToday ? "today" : ""}`}
                >
                  {p.weekday}
                </text>
              );
          ...
          <div className="home-graph-empty">
            ...
            <small>
              Graph updates automatically after each check-in & check-out
              (Mon–Fri).
            </small>
          </div>
        )}
      ...
      <div className="home-graph-foot">
        <span className="foot-left">
          {displayPoints.length
            ? `${displayPoints[0].label} ${displayPoints[0].weekday} — ${displayPoints[displayPoints.length - 1].label} ${displayPoints[displayPoints.length - 1].weekday}`
            : "—"}
        </span>
      ...
      <div className="home-day-chips">
        {displayPoints.map((p) => {
          ...
          return (
            <div
              key={p.iso}
              className={`day-chip ${has ? "has" : ""} ${isToday ? "today" : ""} ${p.status === "LATE" ? "late" : ""}`}
            >
              <b>{p.weekday}</b>
              <span className="t">{fmtMinutes(p.inMin)}</span>
              <small>
                {p.outMin != null
                  ? `out ${shortMinutes(p.outMin)}`
                  : has
                    ? "no out"
                    : "absent"}
              </small>
      ...
      {streak && (
        ...
        >
          🔥 {streak.streak ?? 0} day streak · {streak.present ?? 0} early ·{" "}
          {streak.late ?? 0} late · {streak.checkedOut ?? 0} check-outs
        </div>
      )}
    </div>
  );
...
export default function HomePage() {
  ...
  const name =
    summary?.fullName || localStorage.getItem("ccd.name") || "Student";
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      if (st?.status === "fulfilled" && st.value?.data)
        setStreak(st.value.data);
    });
  }, []);

  const refreshCalendar = () => {
    api
      .get("/student/attendance/calendar?days=14")
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      refreshCalendar();
      api
        .get("/student/attendance/current")
  ...
  const maskedId =
    membership === "—"
      ? "—"
      : membership.startsWith("CCD")
        ? membership.slice(0, 7) + " •••"
        : "•••• ••••";
    ...
    <div className="edu-home">
      ...
      <section className="edu-hero-card">
        <div className="edu-hero-main">
          <h1>
            {campusGreeting(clock)}, {firstName}
          </h1>
          {checkedIn && (
          ...
          <p className="edu-hero-date">
            {formatCampusDate(clock)} · {formatCampusTime(clock)}
          </p>
          <div className="edu-pills">
            <span className="edu-pill">
              <b>{streak?.present ?? 0} early</b> · {streak?.late ?? 0} late
            </span>
            <span className="edu-pill edu-pill-accent">
        ...
        <div className="edu-hero-side">
          ...
          <p className="edu-hero-hint">
            {checkedIn
              ? `You can check out from ${session?.expectedEnd || "15:00"}`
              : "Tap to open scanner"}
          </p>
        </div>
      </section>
      ...
      <section className="home-quick-grid">
        <Link to="/student/attendance" className="q-tile primary">
          <i>✓</i>
          <span>{checkedIn ? "Check out" : "Check in"}</span>
        </Link>
        <Link to="/student/courses" className="q-tile">
          <i>📚</i>
          <span>Courses</span>
        </Link>
        <Link to="/student/messages" className="q-tile">
          <i>✉</i>
          <span>Messages</span>
        </Link>
        <Link to="/student/profile" className="q-tile">
          <i>👤</i>
          <span>Profile</span>
        </Link>
        <Link to="/student/attendance" className="q-tile">
          <i>◎</i>
          <span>QR Scan</span>
        </Link>
        <Link to="/student/profile" className="q-tile">
