...
export default function AutoCheckInPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        if (!code)
          throw new Error(
            "Missing venue code. Scan the QR displayed in the RAFIC room with your phone camera.",
          );
        ...
        if (!session?.sessionId)
          throw new Error("No active attendance session right now.");
        ...
        setStatus(
          checkingOut
            ? "Confirm you are still in RAFIC to check out."
            : "Confirm you are inside RAFIC to check in.",
        );
        setPhase("ready");
      } catch (err) {
        fail(err);
  ...
  async function completeAttendance() {
    ...
    try {
      ...
      setStatus(
        checkingOut
          ? "Checking your location for check-out…"
          : "Checking your location for check-in…",
      );
  ...
  return (
    <section
      className={`face-id-shell auto-checkin ${phase === "done" ? "stage-success auto-checkin-success" : phase === "error" ? "stage-error" : "stage-intro"}`}
    >
      <div className="face-id-copy" style={{ maxWidth: 520, margin: "0 auto" }}>
        {(phase === "loading" ||
          ...
          phase === "verifying") && (
          <>
            <p className="scan-kicker pulse-text">
              {action === "check-out" ? "AUTO CHECK-OUT" : "AUTO CHECK-IN"}
            </p>
            <h2>{status}</h2>
            {phase === "ready" ? (
              <>
                <p>
                  iPhone: allow location and turn on Precise Location when
                  Safari asks. You must be inside RAFIC.
                </p>
                <button
                ...
                >
                  {action === "check-out"
                    ? "Allow location & check out"
                    : "Allow location & check in"}
                </button>
              </>
            ) : (
              <>
                <p>
                  Stay on this page. We verify your location inside RAFIC, then
        ...
        {phase === "done" && (
          <>
            ...
            <h2 className="auto-checkin-ok-title">
              {celebrating
                ? isOut
                  ? "Congratulations! You successfully checked out"
                  : checkInHeadline
                : status}
            </h2>
            <p>
            ...
            <div className="scan-details auto-checkin-details">
              {[
                ...
                {
                  label: "Student ID",
                  value: displayMembershipId(summary || {}),
                },
                {
                  label: "Registration",
                  value: displayRegistration(summary || {}),
                },
                { label: "Time", value: timeValue },
                {
                  label: "Status",
                  value: isOut ? record?.status || "—" : checkInStatusLabel,
                },
              ].map((d) => (
                <div key={d.label}>
                  <span>{d.label}</span>
