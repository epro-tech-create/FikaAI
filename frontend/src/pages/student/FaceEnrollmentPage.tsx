...
export default function FaceEnrollmentPage({
    ...
    const poses = [
      {
        ...
        status: "Hold a straight pose",
        samples: 2,
        matches: (face: FaceReading) =>
          face.ready && Math.abs(face.yaw) <= 0.09,
      },
      {
        instruction: "Slowly turn your head to the left",
        status: "Hold your left profile",
        samples: 1,
        matches: (face: FaceReading) =>
          face.faceCount === 1 &&
          face.sizeOk &&
      ...
      },
      {
        ...
        status: "Hold your right profile",
        samples: 1,
        matches: (face: FaceReading) =>
          face.faceCount === 1 &&
          face.sizeOk &&
      ...
      },
      {
        ...
        status: "Hold the downward angle",
        samples: 1,
        matches: (face: FaceReading) =>
          face.ready &&
          Math.abs(face.yaw) <= 0.12 &&
          Math.abs(face.pitch - baselinePitch) >= 8,
      },
      {
        instruction: "Return and look straight again",
        status: "Hold the final straight pose",
        samples: 2,
        matches: (face: FaceReading) =>
          face.ready &&
          Math.abs(face.yaw) <= 0.09 &&
          Math.abs(face.pitch - baselinePitch) <= 7,
      },
    ];
    ...
    for (let index = 0; index < poses.length; index++) {
      ...
      while (Date.now() < deadline && runId.current === activeRun) {
        ...
        if (!face.ready) setScanStatus(face.hint);
          ...
          setScanStatus(matches ? `${pose.status} — keep still` : pose.status);

        if (matches) {
          if (!heldFrom) heldFrom = face.analyzedAt;
          const held = face.analyzedAt - heldFrom;
          setProgress(
            7 + index * 13 + Math.min(13, Math.round((held / 1000) * 13)),
          );
          if (held >= 1000) {
            ...
            if (index === 0) {
              baselinePitch = face.pitch;
              setSnapshot(sample.dataUrl);
            }
      ...
      const completedSamples = poses
        .slice(0, index + 1)
        .reduce((total, item) => total + item.samples, 0);
      if (samples.length !== completedSamples)
        throw new Error(
          `The ${pose.instruction.toLowerCase()} pose was not captured in time. Please try again.`,
        );
    }
    return samples;
  }
  ...
  async function enroll() {
    ...
    try {
      ...
      monitor.stop();
      cam.stop();
      setInstruction("Encrypting your Face ID…");
      setScanStatus("Building your multi-angle biometric profile");
      setProgress(72);
      processingTimer = window.setInterval(
        () => setProgress((value) => Math.min(94, value + 1)),
        130,
      );
      const response = await api.post("/student/face-enrollment", {
        samples,
    ...
    <>
      {!embedded && (
        <section className="hero compact-hero">
          <p className="eyebrow">BIOMETRIC IDENTITY SETUP</p>
          <h1>Create your secure Face ID</h1>
          <p className="date">
            Five verified captures across front, left, right and downward angles
            generate one encrypted facial profile.
          </p>
        </section>
      )}
      {stage === "intro" && (
        <label className="consent consent-dark">
          <input
            type="checkbox"
            checked={consent}
          ...
          />
          <span>
            <b>Biometric consent</b>I consent to encrypted face-embedding
            storage for attendance verification.
          </span>
        </label>
      )}
      <FaceScanFlow
        ...
        error={error}
        introTitle={
          status?.enrolled ? "Update your Face ID" : "Enrol your Face ID"
        }
        ...
        details={[
          {
            label: "Face ID",
            value: status?.faceId ? `${status.faceId.slice(0, 8)}…` : "Created",
          },
          { label: "Samples", value: String(status?.sampleCount || 5) },
          {
            label: "Provider",
            value: status?.provider?.startsWith("insightface_")
              ? "ArcFace"
              : status?.provider || "ArcFace",
          },
        ...
        onReset={reset}
        onSuccess={() =>
          onEnrolled ? onEnrolled() : navigate("/student/attendance")
        }
      />
    </>
  );
