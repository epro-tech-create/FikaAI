...
export default function AttendancePage() {
  ...
  const { clock, skewMs, isSynced } = useServerClock();
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      ([sessionResult, summaryResult, enrollmentResult, attendanceResult]) => {
        ...
        if (attendanceResult.status === "fulfilled")
          setRecord(attendanceResult.value.data.record);
        const failure = [
          sessionResult,
          summaryResult,
          enrollmentResult,
  ...
  async function waitForPosition(activeRun: number) {
    let heldFrom = 0,
      lastSeq = monitor.current.current.sequence,
      prev = 0;
    ...
    while (Date.now() < deadline && runId.current === activeRun) {
      ...
      if (!isFreshReading(face, lastSeq, now)) {
        if (prev && now - prev > 350) {
          heldFrom = 0;
          setScanStatus("Camera paused — hold still");
        }
        await sleep(70);
        continue;
      }
      lastSeq = face.sequence;
      if (!isContinuousReading(face, prev)) heldFrom = 0;
      prev = face.analyzedAt;
    ...
    throw new Error(
      "Your face was not positioned in time. Center your face and try again.",
    );
  }
  async function captureChallenge(
    ...
    const capture = (at: number) => {
      ...
      const f = cam.grabFrame(lastVideoTime);
      lastVideoTime = f.videoTime;
      frames.push(f.dataUrl);
      if (frames.length === 1) setSnapshot(f.dataUrl);
      lastCapturedAt = at;
    };
    while (
      performance.now() - startedAt < 22000 &&
      ...
      const now = performance.now();
      if (!isFreshReading(face, lastSeq, now)) {
        if (prev && now - prev > 350) straightHeldFrom = 0;
        await sleep(70);
        continue;
      }
      lastSeq = face.sequence;
      if (!isContinuousReading(face, prev)) straightHeldFrom = 0;
      prev = face.analyzedAt;
      ...
      if (
        face.analyzedAt - lastCapturedAt >= 280 &&
        frames.length < MAX_CAPTURE_FRAMES
      )
        capture(face.analyzedAt);
      if (challengeType === "BLINK_TWICE") {
        if (face.blink >= 0.55 && !eyesClosed) {
          eyesClosed = true;
          blinkPeaks += 1;
          if (face.analyzedAt !== lastCapturedAt) capture(face.analyzedAt);
        }
        if (face.blink <= 0.42) eyesClosed = false;
        actionCompleted = blinkPeaks >= 2;
        setScanStatus(
          actionCompleted
            ? "Two blinks detected — hold still"
            : `Blink detected ${blinkPeaks} of 2`,
        );
      } else if (challengeType === "SMILE") {
        if (
          !actionCompleted &&
          face.smile >= 0.45 &&
          face.analyzedAt !== lastCapturedAt
        )
          capture(face.analyzedAt);
        actionCompleted = actionCompleted || face.smile >= 0.45;
        setScanStatus(
          actionCompleted
            ? "Smile detected — hold still"
            : "Waiting for a clear smile",
        );
        ...
        setScanStatus(
          actionCompleted
            ? "Right turn detected — hold still"
        ...
        setScanStatus(
          actionCompleted
            ? "Straight pose confirmed — hold still"
            : "Keep looking straight",
        );
      }
      setInstruction(
        actionCompleted ? "Completing secure face scan…" : challengeInstruction,
      );
      setProgress(
        Math.min(
          70,
          20 +
            Math.round((Math.min(elapsed, 4000) / 4000) * 30) +
            (actionCompleted
              ? 20
      ...
      if (actionCompleted && elapsed >= 4000 && frames.length >= 12)
        return frames;
      await sleep(70);
    }
    throw new Error(
      "The requested face action was not completed in time. Please try again.",
    );
  }
  ...
  async function scanFace() {
    ...
    // Use server EAT clock (not phone Date.now()) — spoofing phone time no longer opens window
    const nowMs = clock.getTime();
    if (checkingOut) {
      const w = checkoutWindow(
        session.checkoutOpensAt,
        session.checkoutClosesAt,
        nowMs,
      );
      if (w?.state === "before") {
        setError(`Checkout opens at ${formatCampusTime(w.opensAt)}.`);
        return;
      }
      if (!w || w.state === "closed") {
        setError(
          `Checkout closed at ${formatCampusTime(session.checkoutClosesAt)}.`,
        );
        return;
      }
    } else if (nowMs > new Date(session.checkInCloseAt).getTime()) {
      setError(
        `Check-in closed at ${formatCampusTime(session.checkInCloseAt)}.`,
      );
      return;
    }
    ...
    setStage("scanning");
    setProgress(2);
    setInstruction("Loading secure face scanner…");
    setScanStatus("Allow camera access when asked");
    setError("");
    ...
    try {
      ...
      setProgress(18);
      setScanStatus("Face locked");
      const ch = await api.post("/student/liveness/challenge", {
        sessionId: session.sessionId,
      });
      const ct = parseChallengeType(ch.data?.challengeType);
      const ci =
        typeof ch.data?.instruction === "string" && ch.data.instruction.trim()
          ? ch.data.instruction
          : "Complete the requested face action";
      const frames = await captureChallenge(activeRun, ct, ci);
      monitor.stop();
      cam.stop();
      setInstruction("Verifying liveness and identity…");
      setScanStatus("Checking encrypted biometric profile");
      setProgress(72);
      timer = window.setInterval(
        () => setProgress((v) => Math.min(93, v + 1)),
        130,
      );
      const verified = await api.post("/student/attendance/verify-face", {
        sessionId: session.sessionId,
        challengeToken: ch.data.challengeToken,
        frames,
      });
  ...
  const checkInClosed =
    !checkedIn &&
    Boolean(
      session && clock.getTime() > new Date(session.checkInCloseAt).getTime(),
    );
  const scheduleLocked = checkoutLocked || checkInClosed;
  const checkoutOpenTime = checkout
    ? formatCampusTime(checkout.opensAt)
    : "2:00 PM";
  const checkoutCloseTime = checkout
    ? formatCampusTime(checkout.closesAt)
    : "4:00 PM";
  ...
  const introText = checkInClosed
    ...
    : checkout?.state === "before"
      ...
      : checkout?.state === "closed"
        ? `Today’s checkout window was ${checkoutOpenTime} to ${checkoutCloseTime}.`
        : checkedIn
          ? "Complete a live Face ID scan to check out — or scan the room QR with your phone camera."
          : "Preferred: scan the room QR with your phone camera. Face ID below is the in-app option.";
  const actionLabel =
    checkout?.state === "before"
      ? `Checkout at ${checkoutOpenTime}`
      : checkout?.state === "closed"
        ? "Checkout closed"
        : checkInClosed
          ? "Check-in closed"
  ...
  if (pendingVenue)
    return <Navigate to={studentCheckinPath(pendingVenue)} replace />;
  ...
  return (
    <div>
      <div className="student-checkin-head">
        ...
        <p>
          Scan the QR in the RAFIC room with your phone camera. After login, the
          system checks GPS and confirms check-in or check-out automatically.
        </p>
      </div>
      {skewMs > 120000 && (
        <div
        ...
        >
          Phone clock off by {Math.round(skewMs / 60000)} min vs campus time
          (EAT). Window uses server time. {isSynced ? "Synced." : "Syncing…"}
        </div>
      )}
      {error && stage === "intro" && <div className="error">{error}</div>}

      {!enrolled && stage === "intro" && (
        <div className="error" style={{ marginBottom: 12 }}>
          ...
          to use Face ID here, or scan the room QR with your phone.
        </div>
      )}
      <FaceScanFlow
        stage={stage}
        ...
        error={error}
        errorTitle={
          checkedIn ? "Check-out unsuccessful" : "Check-in unsuccessful"
        }
        ...
        successText={
          record?.status === "CHECKED_OUT"
            ? "Your live face matched the encrypted profile successfully."
            : record?.status === "LATE"
              ? "Your arrival is verified. Official start is 09:30, so this check-in is late."
              : "You arrived before 09:30, so this check-in is early."
        }
        details={[
          ...
          {
            label: "Face ID",
            value: record?.faceId ? `${record.faceId.slice(0, 8)}…` : "—",
          },
          {
            label: "Time",
