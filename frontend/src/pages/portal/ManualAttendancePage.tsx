...
export default function ManualAttendancePage({
    ...
    async function load() {
      ...
      try {
        ...
        setStudents(
          sData.map((x: any) => ({
            id: String(x.id),
            fullName: x.fullName || x.full_name || x.studentName,
            membershipId: x.membershipId,
            registrationNumber: x.registrationNumber || x.registration_number,
            email: x.email,
          })),
        );
        setSessions(mapped);
  ...
  async function saveHours() {
    ...
    try {
      ...
      setOk(
        `Session hours saved: check-in ${toTimeInput(updated.checkInOpen)}–${toTimeInput(updated.checkInClose)}, checkout ${toTimeInput(updated.expectedEnd)}–${toTimeInput(updated.checkOutClose)}`,
      );
    } catch (e) {
      setError(message(e));
    } finally {
  ...
  async function doCheckIn() {
    ...
    try {
      ...
      setOk(
        `Checked in: ${res.data?.status || status} · in ${formatSaved(res.data?.checkInAt)}${res.data?.checkOutAt ? ` · out ${formatSaved(res.data.checkOutAt)}` : ""}`,
      );
    } catch (e) {
      setError(message(e));
    } finally {
  ...
  async function doExcuse() {
    ...
    try {
      ...
      if (!recId && studentId && sessionId) {
        ...
        if (!found)
          throw new Error(
            "No attendance record for that student+session. Check-in first or create ABSENT then excuse.",
          );
        recId = String(found.id);
      }
      if (!recId)
        throw new Error(
          "Provide record ID or pick student+session with existing record",
        );
      if (!excuseReason.trim())
        throw new Error("Provide reason: e.g. sickness, funeral of dad/mom");
      const res = await api.post(`/${role}/attendance/${recId}/excuse`, {
        reason: excuseReason,
        status: "EXCUSED",
      });
      setOk(
        `Excused: ${res.data?.status} - ${res.data?.excuseReason || excuseReason}`,
      );
    } catch (e) {
      setError(message(e));
    } finally {
  ...
  return (
    <main className="portal-content">
      <PageHeading
        ...
        description="Change today’s check-in / checkout windows, or write a student’s arrival and departure times by hand."
      />
      ...
      >
        ...
        >
          This is the 08:00–15:00 / 15:00–17:00 window students see. Edit it
          here and save — it stays until you change it again.
        </p>
          ...
          >
            ...
            {sessions.map((s) => (
              <option key={s.id} value={s.id}>
                {s.session_date || "—"} — {s.title} ({s.status})
              </option>
            ))}
          </select>
        </label>
      ...
      >
        ...
        >
          ...
          {status === "EXCUSED" && (
            <label style={{ flex: 1, ...labelStyle }}>
              Reason (sickness, funeral of dad/mom...)
              <input
                value={reason}
                onChange={(e) => setReason(e.target.value)}
                placeholder="e.g. sickness, funeral"
                style={fieldStyle}
              />
            </label>
          )}
        ...
        >
          Per-student times ignore the 08:00–15:00 window. Leave a field empty
          to use now. Fill both, then Save check-in, to record arrival and
          departure together.
        </small>
      </section>
      <section
      ...
      >
        ...
        >
          Mark an existing record as EXCUSED with reason (sickness, funeral
          etc). Or clear excuse to revert to PRESENT.
        </p>
        <label style={labelStyle}>
          Record ID (or leave blank to auto-find via student+session above)
          <input
            value={excuseRecordId}
            onChange={(e) => setExcuseRecordId(e.target.value)}
            placeholder="uuid of attendance record"
            style={fieldStyle}
          />
        </label>
        <label style={{ ...labelStyle, marginTop: 12 }}>
          ...
          <input
            ...
            placeholder="e.g. sickness, mother funeral"
            style={fieldStyle}
          />
        </label>
        <div
          style={{ marginTop: 16, display: "flex", gap: 10, flexWrap: "wrap" }}
        >
          <button disabled={busy} onClick={doExcuse} className="portal-primary">
            Set EXCUSED
          </button>
          <button
            disabled={busy}
