...
export default function ManualAttendancePage({ role }: { role: Extract<Role,'admin'|'instructor'> }) {
  ...
  return <main className="portal-content">
    ...
    <section className="content-card" style={{marginBottom:16}}>
      <h4>Pick student & session</h4>
      <div style={{display:'grid',gap:12,gridTemplateColumns:'1fr 1fr'}}>
        <label>Student ({students.length})
          <select value={studentId} onChange={e=>setStudentId(e.target.value)} style={{width:'100%',padding:8}}>
        ...
        <label>Session ({sessions.length})
          <select value={sessionId} onChange={e=>setSessionId(e.target.value)} style={{width:'100%',padding:8}}>
            <option value="">-- pick --</option>
      ...
      <div style={{marginTop:12,display:'flex',gap:12,flexWrap:'wrap',alignItems:'center'}}>
        ...
        {status==='EXCUSED' && <label style={{flex:1}}>Reason (sickness, funeral of dad/mom...)<input value={reason} onChange={e=>setReason(e.target.value)} placeholder="e.g. sickness, funeral" style={{width:'100%',padding:8}}/></label>}
      </div>
      ...
      <small style={{color:'#64748b'}}>Uses POST /{role}/attendance/manual-check-in and manual-check-out with MANUAL verification.</small>
    </section>
    <section className="content-card">
      <h4>Excuse / acceptable reason</h4>
      <p style={{color:'#475569',fontSize:13}}>Mark an existing record as EXCUSED with reason (sickness, funeral etc). Or clear excuse to revert to PRESENT.</p>
      <label>Record ID (or leave blank to auto-find via student+session above)
        <input value={excuseRecordId} onChange={e=>setExcuseRecordId(e.target.value)} placeholder="uuid of attendance record" style={{width:'100%',padding:8}}/>
      </label>
      <label style={{display:'block',marginTop:8}}>Reason
        <input value={excuseReason} onChange={e=>setExcuseReason(e.target.value)} placeholder="e.g. sickness, mother funeral" style={{width:'100%',padding:8}}/>
      </label>
      <div style={{marginTop:12,display:'flex',gap:8}}>
