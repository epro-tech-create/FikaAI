...
describe("instructor registration form", () => {
  ...
  it("allows no replacement password when editing", () => {
    expect(
      instructorFormError({ password: "", confirmPassword: "" }, true),
    ).toBe("");
  });

  it("validates an optional replacement password when provided", () => {
    expect(
      instructorFormError({ password: "weak", confirmPassword: "weak" }, true),
    ).toContain("uppercase");
    expect(
      instructorFormError(
        { password: "SecurePass9", confirmPassword: "SecurePass8" },
        true,
      ),
    ).toBe("Passwords do not match.");
  });
