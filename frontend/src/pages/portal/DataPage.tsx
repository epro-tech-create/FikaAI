...
export default function DataPage({ config }: { config: DataPageConfig }) {
  ...
  return (
    <main className="portal-content">
      <PageHeading
        eyebrow={config.eyebrow || "OPERATIONS"}
        title={config.title}
      ...
      {isAuditLogs && (
        <section className="content-card" style={{ marginBottom: 16 }}>
          ...
          >
            ...
            />
            <button className="secondary-button" onClick={bulkDelete}>
              Delete before date
            </button>
      ...
      <section className="content-card">
        ...
        {loading ? (
          ...
          <StatePanel kind="empty">
            {items.length ? "No records match this search." : undefined}
          </StatePanel>
        )}
      </section>
    </main>
