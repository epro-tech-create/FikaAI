...
type Conv = {
  id: string;
  title: string;
  peer: { id: string; fullName: string; email: string; role: string } | null;
...
export default function ChatPage() {
  ...
  async function loadConvs() {
    try {
      ...
      if (Array.isArray(r.data)) setConvs(r.data);
    } catch {}
  }
    ...
    const h = setTimeout(async () => {
      try {
        const r = await api.get(
          `/chat/users/search?q=${encodeURIComponent(q)}&limit=12`,
        );
        setUserSearch(r.data);
      } catch {
        setUserSearch([]);
      }
    }, 280);
  ...
  const filteredConvs = searchQ.trim()
    ? convs.filter(
        (c) =>
          c.title.toLowerCase().includes(searchQ.toLowerCase()) ||
          c.peer?.email.toLowerCase().includes(searchQ.toLowerCase()),
      )
    : convs;
    ...
    >
      <div className="portal-heading">
        <div>
          ...
          <h1>Chat</h1>
          <span>
            WhatsApp-like help desk — students ask, instructors & admins reply.
          </span>
        </div>
        <button
      ...
      <div
        ...
        style={{ height: "calc(100dvh - 300px)", minHeight: 480 }}
      >
          ...
          <div className="chat-convs">
            {filteredConvs.length === 0 && (
              <div className="chat-empty">
                No chats yet. Start a chat with a student or instructor.
              </div>
            )}
            {filteredConvs.map((c) => (
              <button
              ...
              >
                ...
                <span className="chat-conv-main">
                  ...
                  <small>
                    {c.lastMessage?.body.slice(0, 38) || "No messages yet"}
                  </small>
                </span>
                {c.unreadCount > 0 && (
          ...
          {!activeId ? (
            <div className="chat-welcome">
              <div className="chat-welcome-card">
                ...
                <h3>Chat help desk</h3>
                <p>
                  Select a conversation or start a new one. Students can message
                  instructors/admins for help; replies appear here in real time
                  (polling — WS upgrade ready).
                </p>
              </div>
            </div>
          ) : (
            <>
              <div className="chat-header">
                ...
                <span className="chat-header-info">
                  ...
                  <small>
                    {activeConv?.peer
                      ? `${activeConv.peer.role} · ${activeConv.peer.email}`
                      : "Group"}
                  </small>
                </span>
              </div>
