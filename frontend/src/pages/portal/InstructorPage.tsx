...
export function instructorFormError<
  T extends { password: string; confirmPassword: string },
>(form: T, isEditing = false) {
...
export default function InstructorPage() {
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      ...
      closeForm();
      setNotice(
        `${name} was ${wasEditing ? "updated" : "registered"} successfully.`,
      );
      await load();
    } catch (requestError) {
  ...
  async function remove(instructor: Instructor) {
    if (
      !window.confirm(
        `Delete ${instructor.fullName} permanently? This cannot be undone.`,
      )
    )
      return;
  ...
  const visibleInstructors = instructors.filter((instructor) =>
    matchesSearch(instructor, searchQuery, ["fullName", "email"]),
  );
  ...
  return (
    <main className="portal-content">
      <PageHeading
        ...
        title="Instructors"
        description="Register instructor accounts and manage access to the teaching portal."
        action={
          <button
          ...
          >
            {showForm ? "Close form" : "Register instructor"}
          </button>
        }
      />
      {showForm && (
        <form className="content-card session-form" onSubmit={save}>
          <div className="form-heading">
            <div>
              <p>{editing ? "EDIT INSTRUCTOR" : "NEW INSTRUCTOR"}</p>
              <h2>
                {editing ? "Update instructor account" : "Instructor account"}
              </h2>
            </div>
            <span>
              {editing
          ...
          <div className="form-grid">
            ...
            {editing && (
              <label className="checkbox-control wide">
                ...
                <span>
                  <b>Active account</b>
                  <small>Instructor can sign in</small>
                </span>
              </label>
            )}
            <label className="wide">
              {editing ? "Replacement password (optional)" : "Initial password"}
              <input
                required={!editing}
      ...
      <section className="content-card">
        <CardToolbar
          ...
          meta={
            searchQuery
              ? `${visibleInstructors.length} of ${instructors.length} records`
              : `${instructors.length} records`
          }
          search={{
            value: searchInput,
        ...
        {loading ? (
          ...
          <DataTable
            columns={[
              { key: "fullName", label: "Instructor" },
              { key: "email", label: "Email" },
              { key: "isActive", label: "Status" },
              { key: "createdAt", label: "Created" },
            ]}
            items={visibleInstructors}
            renderActions={(item) => {
              const instructor = item as Instructor;
              return (
                <>
                  <button onClick={() => startEdit(instructor)}>Edit</button>
                  <button
          ...
          !error && (
            <StatePanel kind="empty">
              {instructors.length
                ...
                : "Register the first instructor account to begin."}
            </StatePanel>
          )
        )}
      </section>
