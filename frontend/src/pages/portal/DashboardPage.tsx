...
export function chartCoords(
  ...
  return points.map((point, index) => {
    ...
    const y = clampChartY(
      chartBottom - (Math.max(0, point[key]) / Math.max(1, maximum)) * span,
    );
    return { x, y };
  });
}
...
export function chartPolyline(
  ...
  return chartCoords(points, key, maximum)
...
export default function DashboardPage({
  ...
  const maximum = Math.max(
    1,
    ...timeline.flatMap((point) => [point.arrivals, point.departures]),
  );
  ...
  const fillPath = arrivalPath
    ? `${arrivalPath} L ${chartRight} ${chartBottom} L ${chartLeft} ${chartBottom} Z`
    : "";
  ...
  return (
    <main className="portal-content">
      <PageHeading
        ...
        title={`${campusGreeting(clock)}, ${(localStorage.getItem("fikaai.name") || role).split(" ")[0]}`}
        description="Today’s attendance at DIT RAFIC Building. The graph refreshes automatically every 10 seconds."
      />
      {loading ? (
        ...
        <>
          {role === "admin" && (
            <section className="stat-grid" aria-label="Daily student summary">
              <StatCard
                label="Registered students"
                value={data.students}
                note="All student accounts"
              />
              <StatCard
                label="Checked in today"
                value={data.arrivalsToday}
                note="Verified arrivals today"
                tone="accent"
              />
              <StatCard
                label="Face enrolled"
                value={data.activeFaceEnrollments}
                note="Active Face ID profiles"
              />
              <StatCard
                label="Checked out today"
                value={data.departuresToday}
                note="Recorded departures today"
              />
            </section>
          )}
          {role === "instructor" && (
          ...
          <section className="content-card attendance-chart-card">
            <div className="attendance-chart-head">
              <div>
                ...
                <h2>{String(data.date || "Today")}</h2>
              </div>
              <div className="chart-totals">
                <span>
                  <i className="arrivals" />
                  Check-in <b>{String(data.arrivalsToday ?? 0)}</b>
                </span>
                <span>
                  <i className="departures" />
                  Check-out <b>{String(data.departuresToday ?? 0)}</b>
                </span>
                <small>Live</small>
              </div>
            </div>
            <div className="attendance-chart-wrap">
              <svg
                ...
                role="img"
                aria-label="Student check-in and check-out today"
              >
                <defs>
                  <linearGradient
                    id="attendanceArea"
                    ...
                    y2="1"
                  >
                    <stop offset="0" stopColor="#3b9cff" stopOpacity=".34" />
                    <stop offset="1" stopColor="#3b9cff" stopOpacity=".02" />
                  </linearGradient>
                  <clipPath id="attendancePlot">
                    <rect
                      x={chartLeft}
                      y={chartTop}
                ...
                <g clipPath="url(#attendancePlot)">
                  ...
                  <polyline
                    className="chart-line arrivals"
                    points={arrivalPoints}
                  />
                  <polyline
                    className="chart-line departures"
