...
const content: Record<string, { description: string; cards: [string, string][] }> = {
  ...
  Profile: { description: 'Your portal identity and access context.', cards: [['Signed in as', localStorage.getItem('ccd.name') || 'Portal user'], ['Account security', 'Your session is protected by a short-lived access token and stays active until you sign out.'], ['Role access', `You are signed in with the ${localStorage.getItem('ccd.role') || 'assigned'} role.`]] },
}
