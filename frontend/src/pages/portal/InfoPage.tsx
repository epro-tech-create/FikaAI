...
const content: Record<string, { description: string; cards: [string, string][] }> = {
  ...
  Profile: { description: 'Your portal identity and access context.', cards: [['Signed in as', localStorage.getItem('fikaai.name') || 'Portal user'], ['Account security', 'Your session is protected by a short-lived access token and ends when this app is refreshed.'], ['Role access', `You are signed in with the ${localStorage.getItem('fikaai.role') || 'assigned'} role.`]] },
}
