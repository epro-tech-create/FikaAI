...
export function studentFormError(form: StudentForm, isEditing = false) {
  if (!/^\d{3,50}$/.test(form.registrationNumber))
  ...
  return (
    <main className="portal-content">
      <PageHeading
        ...
        title="Students"
        description="Manage student identity, registration details, and account access."
        action={
          <button className="portal-primary" onClick={startCreate}>
            Add student
          </button>
        }
      />
      ...
      <section className="content-card">
        <CardToolbar
          ...
          meta={
            searchQuery
              ? `${visibleStudents.length} of ${students.length} records`
          ...
          search={{
            ...
            placeholder: "Name, student ID, registration…",
        ...
        {loading ? (
          ...
          <DataTable
            ...
            renderActions={(item) => {
              ...
              return (
                <>
                  <button onClick={() => startView(student)}>View</button>
          ...
          !error && (
            <StatePanel kind="empty">
              {students.length
                ...
                : "Add the first student account to begin."}
            </StatePanel>
          )
        )}
      </section>
      {showForm && (
        <PortalDialog labelledBy="student-form-title" onClose={closeForm}>
          <form className="session-form student-dialog-form" onSubmit={save}>
            <div className="form-heading">
              <div>
                ...
                <h2 id="student-form-title">
                  {editing ? "Update student account" : "Student account"}
                </h2>
              </div>
              <span>
                {editing
            ...
            <div className="form-grid">
              ...
              <label>
                Student ID <small>(CCD membership)</small>
                <input
                  value={form.membershipId}
                  onChange={(event) =>
                    setForm({
                      ...form,
                      membershipId: normalizeMembershipIdInput(
                        event.target.value,
                      ),
                    })
                  }
              ...
              <label className="checkbox-control">
                ...
                <span>
                  <b>Active account</b>
                  <small>Student can sign in</small>
                </span>
              </label>
              <label className="wide">
                {editing ? "Replacement password (optional)" : "Password"}
                <input
                  required={!editing}
      ...
      {viewing && (
        <PortalDialog
          labelledBy="student-view-title"
          onClose={() => setViewing(null)}
        >
          <section
            className="record-details student-dialog-view"
            aria-label={`${viewing.fullName} details`}
          >
            <div className="form-heading">
              <div>
                <p>STUDENT DETAILS</p>
            ...
            <dl>
              <div>
                <dt>Student ID</dt>
                <dd>{viewing.membershipId || "Not set"}</dd>
              </div>
              <div>
                <dt>Registration</dt>
                <dd>{viewing.registrationNumber}</dd>
              </div>
              <div>
                <dt>Email</dt>
                <dd>{viewing.email}</dd>
